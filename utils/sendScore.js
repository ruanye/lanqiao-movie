...
const newD = data.pass_detail;

const hash = new Map();
// 去除重复得分项，判断标注为 id 相同，则只有一个得分
const RemoveDuplicates = newD => {
  const newArr = [];
  newD.forEach(item => {
    if (item.mark_id) {
      // 判断是否有重复得分的标注信息
      const key = hash.get(item.mark_id);
      if (!key) {
...
const lastdata = RemoveDuplicates(newD);
...
const sendScore = () => {
  ...
  skillPointIdList.forEach(item => {
    ...
    const oneSkillInfo = lastdata.filter(info => info.skill_point_id == item);
    // 定义最终返回的格式
    const skillPassDetail = {
    ...
    const passedScore = oneSkillInfo[0].passed_score;
    ...
    if (
      ...
      skillPassDetail.skill_point_id == 1815 ||
  ...
  request(
    ...
    function (error, response, body) {
      console.log(111, response.statusCode, error);
